      ...
      WebViewer(
        {
          ...
          licenseKey:
            "demo:1724086098724:7e48ccef030000000029d75904e4c73f9ae2edceae82f126abeadb2ed2",
        },
        viewerDiv.current
      ).then((instance) => {
        ...
        const matchContoursToRooms = (contours) => {
          ...
          return contours.map((contour) => {
            ...
            for (const [type, { minArea, maxArea }] of Object.entries(
              roomTypes
            )) {
        ...
        const calculateACPosition = (room, index) => {
          ...
          const totalUnitsInRow = Math.floor(
            room.width / (unitWidth + padding)
          );
        ...
        const determineScale = (pdfPage) => {
          ...
          const referenceBoundingBox =
            pdfPage.getBoundingBoxForReferenceObject();
        ...
        if (cvReady) {
          const loadPDF = async (pdfPath) => {
            const document = await PDFTron.loadDocument(pdfPath);
            return document;
          };

          const processPDF = async (pdfPath) => {
            const document = await loadPDF(pdfPath);
            const pdfPage = getPdfPage(document, 1);
